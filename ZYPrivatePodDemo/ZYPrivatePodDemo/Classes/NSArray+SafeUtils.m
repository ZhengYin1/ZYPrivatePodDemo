...
- (id)mjyp_##arrayName##_objectAtIndex:(NSUInteger)index {\
    ZYAssert([self count] > 0,      ([NSString stringWithFormat:@"index %lu beyond bounds for empty array", (unsigned long)index]));\
    ZYAssert(index < [self count],  ([NSString stringWithFormat:@"index %lu beyond bounds [0...%lu]", (unsigned long)index, (unsigned long)[self count] - 1]));\
    if (index < self.count) {\
...
- (void)mjyp_insertObject:(id)anObject atIndex:(NSUInteger)index {
    ZYAssert(anObject, @"object cannot be nil");
    ZYAssert(index <= [self count], ([NSString stringWithFormat:@"index %lu beyond bounds [0...%lu]", (unsigned long)index, (unsigned long)[self count] -1]));
