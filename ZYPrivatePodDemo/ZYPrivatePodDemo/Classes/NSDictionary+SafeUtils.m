...
+ (instancetype)mjyp_dictionaryWithObjects:(const id [])objects forKeys:(const id<NSCopying> [])keys count:(NSUInteger)cnt {
    ...
    for (NSUInteger i = 0; i < cnt; i++) {
        ...
        id obj = objects[i];
        if (!key) {
            ZYAssert(key, ([NSString stringWithFormat:@"attempt to insert nil key from key[%lu]", (unsigned long)i]));
            continue;
        }
        if (!obj) {
            ZYAssert(obj, ([NSString stringWithFormat:@"attempt to insert nil object from objects[%lu]", (unsigned long)i]));
            continue;
        }
        safeKeys[j] = key;
        safeObjects[j] = obj;
...
- (void)mjyp_setObject:(id)anObject forKey:(id<NSCopying>)aKey {
    ZYAssert(anObject, ([NSString stringWithFormat:@"object cannot be nil (key = %@)", aKey]));
    ZYAssert(aKey, ([NSString stringWithFormat:@"key cannot be nil (object = %@)", anObject]));
...
- (void)mjyp_setObject:(id)obj forKeyedSubscript:(id<NSCopying>)key {
    ZYAssert(key, ([NSString stringWithFormat:@"key cannot be nil (object = %@)", obj]));
...
- (void)mjyp_removeObjectForKey:(id)aKey {
    ZYAssert(aKey, @"key cannot be nil");
